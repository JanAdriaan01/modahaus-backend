...
const db = require('../_lib/db.cjs'); // use .cjs
const crypto = require('crypto');
const { hash, compare } = require('bcrypt');
const { signAccess, signRefresh, verifyRefresh } = require('../_lib/jwt.cjs'); // ensure .cjs
const { sendEmail } = require('../_lib/email.cjs'); // ensure .cjs
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (action === 'register' && req.method === 'POST') {
      ...
      try { await sendEmail({ to: user.email, subject: 'Verify your email', html }); } catch (err) { console.error('sendEmail failed:', err); }
    ...
    if (action === 'request-reset' && req.method === 'POST') {
      ...
      try { await sendEmail({ to: user.email, subject: 'Reset your password', html }); } catch (err) { console.error('sendEmail failed:', err); }
